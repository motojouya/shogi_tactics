...
- **S23. form モデルの実利用化(creation の配線 + formation の form 化)**【済】(ユーザー指摘: S14 で作った `form/creation` が未使用) — 
  - **`form/creation.ts` を実利用へ**: 未使用だった `normalCreationFormSchema`/`warCreationFormSchema` を、`mode` を含む単一の **`creationFormSchema`**(`mode` enum + player名必須 + `superRefine` で war 時のみ stepBase/unitCount を1以上検証)へ統合。`feature/creation.tsx` を **react-hook-form + `zodResolver(creationFormSchema)`** に書き換え、手書きバリデーション(`register_` の messages 集計)を全廃。エラーは各 TextField の helperText へ。
  - **stepBase/unitCount は `number` 型**(後続のユーザー要望): schema を `z.number({ error })` にし、画面は `register('stepBase', { valueAsNumber: true })` + `defaultValues` を通常モード値 **stepBase=14/unitCount=7**(`NORMAL_STEP_BASE`/`NORMAL_UNIT_COUNT`)で与える(通常モードは入力欄を出さずこの既定値を使用)。利用側の `Number()` 変換を撤去し `form.stepBase`/`form.unitCount` を直接 `registerBattle` へ。
  - **`form/formation.ts` 新設**: 「駒を1体追加」フォームの **`formationFormSchema`**(`{ piece: string(min1), leader: boolean }`)+ **`pieceSelectOption(piece): SelectOption`**。`feature/formation.tsx` を RHF + `zodResolver` 化し、駒 Select と大将 Checkbox を `Controller` 管理、追加は `handleSubmit(addUnit)`、ドロップダウンは `pieceSelectOption` 生成に。`units` は引き続き useState(蓄積する画面状態)。
  - **React Compiler 対応**: react-hook-form の `watch` は React Compiler 非互換警告(`--max-warnings=0` で失敗)になるため、`action.tsx` の既存方針(`getValues` + 別 useState)に倣い、条件描画用の値(creation の `mode`/formation の `selectedPiece`)は Select の onChange でミラー更新する **useState** で保持。
  - 各 form の colocation テスト追加/更新(creation 5・formation 3)。build/test 125 pass/lint/format green。
